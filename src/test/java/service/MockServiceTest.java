...
public class MockServiceTest {
    ...
    @Test
    public void 챔피언포지션을가져오면_무조건_서폿을_리턴한다(){
        Champion champion = mock(Champion.class);
        when(champion.getPosition()).thenReturn("서폿");
        assertThat(champion.getPosition(), is("서폿"));
        System.out.println("Champion : " + champion.getPosition());
    }
    ...
    public void shouldExceptionWhenSetNameYasuo() {
        Champion champion = mock(Champion.class);
        doThrow(new IllegalAccessException()).when(champion).setName("야스오");
    ...
    public void shouldNeverInvocationWhenChampionSetName(){
        ...
        verify(champion, never()).setPosition(anyString());
    }
    ...
    public void 챔피언정보들을Mocking하고Service메소드호출테스트() {
        ...
        //mockRepository로 해줘야한다(얘가 findByName을 호출하기 때문)
